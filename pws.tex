...
In dit diagram raakt lichtstraal $\overrightarrow{L}$ het object in P met hoek $\alpha$ tot normaal $\overrightarrow{N}$. De lichtstraal heeft intensiteit $I$, en is verspreidt over oppervlakte $A$. Dit betekent dat de intensiteit per eenheid oppervlakte $\frac{I}{A}$ is. In een rechte driehoek geldt: 
\[\cos{\alpha}=\frac{A}{S}\]
...
\begin{axis}[
	...
	axis x line=center,
	axis y line=center,
	legend style={at={(1.1,1)},anchor=west, title={$y = \left(\frac{\cos(x) + 1}{2}\right)^n$}},
	legend entries={$n=1$, $n=2$, $n=10$, $n=50$, $n=100$, $n=500$},
]
\addlegendentry[red]{$n=1$};
\addlegendentry[green]{$n=2$};
\addlegendentry[blue]{$n=10$};
\addlegendentry[purple]{$n=50$};
\addlegendentry[orange]{$n=100$};
\addlegendentry[brown]{$n=500$};
\addplot[color=red, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^1};
\addplot[color=green, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^2};
\addplot[color=blue, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^10};
\addplot[color=purple, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^50};
\addplot[color=orange, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^100};
\addplot[color=brown, thick, samples=200, domain=-pi:pi] {((cos(deg(x)) + 1) / 2)^500};
\end{axis}
\end{tikzpicture}
